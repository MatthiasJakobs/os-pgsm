...
drift.topk.models.input=function(models,data.train, data.test,t,val.length, formula, per.arima,ker=c("rbfdot" ,"polydot" ,"vanilladot", "laplacedot"))
{
  ...
  data.val.n= data.val
    ...
    data.val.n=data.test[(t-val.length):(t-1),1:ncol(data.val)]}else{
      target=c(data.val[((t):val.length),1 ],data.test[1:(t-1),1])
      data.val.n=rbind(data.val[((t):val.length), ],data.test[1:(t-1),1:ncol(data.val)])
    }
...
{  
  ...
  preds=sapply(1:length( pred.list), function(x) pred.list[[x]])
  pred=mean(sapply(1:length( pred.list), function(x) pred.list[[x]][t]))
...
calculensemble1=function(H,pr,list.pred, real){
...
ens.step.all.s=function(data.test,predictions.table,t,H)
{  
  if(t<(H+1)){pred=predictions.table[t,2]}else{  
    pred.list=lapply((list.model[[1]]+1), function(x)predictions.table[,x])
    
    pred=calculensemble1(H,t,pred.list,predictions.table[,1])
...
{ 
  ...
  preds=sapply(1:length( pred.list), function(x) pred.list[[x]])
  pred[t]=mean(sapply(1:length( pred.list), function(x) preds[t,x]))
  t=t+1
  repeat{
    if(alarm[t]==1) {
      output.m=drift.topk.models.input(models,data.train, data.test,t,val.length, formula, per.arima,ker=c("rbfdot" ,"polydot" ,"vanilladot", "laplacedot"))
  ...
  plot(data.test$y, type='l')
  lines(pred, col='red')
  
  print( rmse(data.test$y,pred))
...
calculate_ets_pred=function(timeseries,x){
  #datapred=timeseries[(per+n*(day-1)):((ndays*n)+(per-1)+n*(day-1))]
  datapred=timeseries[(1:(x-1))]
  r <- as.numeric(forecast(ets(datapred))$upper[1,1])
...
calculate_ets_prediction=function(train.ts, test.ts,H,x){
  ...
  #ts_train= data[(1+H*(x-1)):(n+H*(x-1)),]
  ts_train= data[1:(n+n.test),]
  
  #datapred=data[(1+H*(x-1)):(n+H*(x)),]
  r=calculate_ets_pred(ts_train,(n+x-1))
 # r=sapply(1:H, function(y) calculate_ets_pred(data[(1+H*(x-1)):(n+H*(x-1)+y),],(n+y)))
  return(r)
}
