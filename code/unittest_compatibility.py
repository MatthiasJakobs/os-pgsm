...
ospgsm_st_original_config = ospgsm_st_original()
lag = 5
ts_length = lag 
...
torch.manual_seed(0)
new_comp = new_OSPGSM(models, ospgsm_int_config, random_state=1010)
...
print("OSPGSM-ST New")
torch.manual_seed(0)
new_comp = new_OSPGSM(models, ospgsm_st_original_config, random_state=1010)
new_preds = new_comp.run(X_val, X_test)
...
ospgsm_per_original_config = ospgsm_per_original()
...
torch.manual_seed(0)
new_comp = new_OSPGSM(models, ospgsm_per_original_config, random_state=1010)
new_preds = new_comp.run(X_val, X_test)
